...
def extract_triples_from_chunk(chunk_text: str, page_num: int, model: str, retries: int = 2) -> List[Dict[str, str]]:
    ...
    for attempt in range(retries):
        try:
            ...
            if not hasattr(extract_triples_from_chunk, "debug_printed"):
                print("\nRAW LLM RESPONSE SAMPLE:")
                print(raw_text[:2000])
                print("\n--- END SAMPLE ---\n")
                extract_triples_from_chunk.debug_printed = True
...
def run_kg_extraction(pdf_path: str, model: str = "deepseek-r1:7b"):
    ...
    print(f"Extracted {len(unique)} unique triples.")
